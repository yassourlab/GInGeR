...
class GingerRunnerTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.merged_filtered_fasta = None  # f'{TEST_FILES}/merged_filtered_ref_db.fasta.gz'
    ...
    def test_ginger_e2e_command(self):
    ...
    def test_ginger_e2e_command_skip_kraken(self):
        ...
        result = runner.invoke(run_ginger_e2e,
                               f'{self.short_reads_1} {self.short_reads_1} {self.genes_path} {self.out_dir} --reads-ratio-th {self.read_ratio_th} --metadata-path {self.metadata_path} --max-species-representatives 1'.split(
                                   ' '))
