...
class GingerRunnerTest(unittest.TestCase):
    ...
    def test_ginger_e2e_command_skip_kraken(self):
        ...
        result = runner.invoke(run_ginger_e2e,
                               f'{self.short_reads_1} {self.short_reads_2} {self.genes_path} {self.out_dir} --sample-specific-references {self.merged_filtered_fasta} --species-coverage-threshold {self.coverage_th} --reference-genomes-metadata {self.metadata_path} --max-species-representatives 1 --no-add-plasmid-score --min-context-len 0'.split(
                                   ' '))
